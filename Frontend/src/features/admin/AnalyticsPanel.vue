...
<template>
  <section class="bg-slate-800 rounded-xl p-6 shadow space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-xl font-semibold">Analüütika</h2>
      <p class="text-sm text-slate-400" v-if="analytics">Uuendatud</p>
    </div>

    <div v-if="!analytics" class="text-slate-400 text-sm">Laen analüütikat...</div>
    <div v-else class="space-y-6">
      <div class="flex items-center gap-3">
        <div class="bg-blue-600/20 text-blue-200 border border-blue-500/50 rounded-lg px-4 py-3">
          <p class="text-xs uppercase tracking-wide">Broneeringuid kokku</p>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div class="bg-slate-900 rounded-lg p-4 border border-slate-700">
          <h3 class="font-semibold mb-2 text-slate-200">Broneeringud filmi kohta</h3>
        ...
        <div class="bg-slate-900 rounded-lg p-4 border border-slate-700">
          <h3 class="font-semibold mb-2 text-slate-200">Täitumus</h3>
          <ul class="space-y-1 max-h-48 overflow-y-auto">
